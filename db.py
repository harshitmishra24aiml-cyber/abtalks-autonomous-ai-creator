...
def save_agent(agent_id: str, name: str, domain: str, created_at: str) -> None:
    with connect() as conn:
        conn.execute(
            """
            INSERT INTO agents(agent_id, persona_name, domain, created_at)
            VALUES (?, ?, ?, ?)
            """,
...
def topic_seen(agent_id: str, url: str) -> bool:
    with connect() as conn:
        row = conn.execute(
            """
...
def save_topic(
    ...
    with connect() as conn:
        conn.execute(
            ...
            (topic_key, agent_id, title, url, source, discovered_at,
...
def recent_posts(agent_id: str, limit: int = 12) -> list[dict[str, Any]]:
    with connect() as conn:
        rows = conn.execute(
            """
